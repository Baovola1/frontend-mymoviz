...
function Home() {
  ...
  useEffect(()=> {
      fetch('https://backend-mymoviz-three.vercel.app/movies')
      .then(response => response.json())
      .then (data => {
        const initData = data.movies.map(movie =>{
          const poster = `https://image.tmdb.org/t/p/w500${movie.poster_path}`
          const overview = movie.overview.substring(0,250) + '...'
          return {poster,title:movie.title, voteAverage:movie.vote_average,voteCount:movie.vote_count,overview}
        });
        setApiMovies(initData)
      });
